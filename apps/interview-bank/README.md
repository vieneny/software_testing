...
一个面向个人学习的 FastAPI + Vue 软件测试面试题库，同时提供无需启动服务的单文件离线版。网站把 2025 第一版、10 个 Markdown 专题、2026 公开面经重构题、补充评审题和公开测试/测开资料的去重重构结果组织成可检索、可练习、可复盘的小型应用。

## 当前内容

- 172 道原有核心详解题；
- 175 道来自 2025 第一版大纲、经过净化和答案补全的详解题；
- 24 道 2026-07 最新趋势原创重构题；
- 52 道独立设计并交叉校验的补充详解题；
- 62 道根据小林 Coding 业务、自动化、性能与测开基础资料去重后新增的详解题；
- 共 485 道可筛选、可查看详细答案的题；
- 两份历史大纲和一份旧 HTML，共 495 条逐项审计记录，全部关联到已确认答案；
...
历史 HTML 不会原样进入网站。2025 第一版使用净化后的题意，并优先关联现行评审答案；没有可靠对应项时采用结构化面试框架，项目题和行为题明确使用虚构练习或要求按本人真实情况回答。
...
离线版和平台版使用同一份 `questions.json`，包含全部 485 题，并按“2025 第一版”和四组后续内容划分来源。支持关键词、模块、来源和收藏筛选；收藏只保存在当前浏览器。
...
flowchart LR
    ...
    C["2025 第一版大纲<br/>175 道净化补全题"] --> D
    C --> E["legacy-coverage.json"]
    D --> F["questions.json<br/>485 道统一题库"]
    E --> F
    F --> G["FastAPI 查询与组卷"]
