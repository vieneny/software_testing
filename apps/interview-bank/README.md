...
一个面向个人学习的 FastAPI + Vue 软件测试面试题库，同时提供无需启动服务的单文件离线版。网站把 2025 优化修订版、10 个 Markdown 专题、2026 公开面经重构题、补充评审题和公开测试/测开资料的去重重构结果组织成可检索、可练习、可复盘的小型应用。

## 当前内容

- 172 道原有核心详解题；
- 160 道来自 2025 优化修订版大纲、经过净化并补全详细答案与答题思路的详解题；
- 24 道 2026-07 最新趋势原创重构题；
- 52 道独立设计并交叉校验的补充详解题；
- 62 道根据小林 Coding 业务、自动化、性能与测开基础资料去重后新增的详解题；
- 共 470 道可筛选、可查看详细答案的题；
- 一份优化修订版大纲和一份旧 HTML 答案来源，共 320 条逐项审计记录，全部关联到已确认答案；
- 21 篇小林 Coding 页面全部进入覆盖清单，共核对 672,610 个正文字符；站内标称 1,143 题与当前可见 1,111 题分开记录；
- 来源目录共 101 条，其中 99 条带公开 URL；62 道公开资料重构题均至少有一个独立标准、官方文档或方法原始来源核验；
...
历史 HTML 不会原样进入网站。2025 优化修订版使用净化后的题意，并优先关联现行评审答案；没有可靠对应项时采用结构化面试框架，项目题和行为题明确使用虚构练习或要求按本人真实情况回答。

## 直接打开离线题库

无需安装 Python、Node.js 或启动服务，直接打开：

```text
apps/interview-bank/offline/软件测试离线题库.html
```

离线版和平台版使用同一份 `questions.json`，包含全部 470 题，并按“2025 优化修订版”和四组后续内容划分来源。支持关键词、模块、来源和收藏筛选；收藏只保存在当前浏览器。
...
- 470 道题结构、ID 和来源有效；
- 470 道题逐题通过答题思路、答案深度、解释深度、场景、来源、图片、代码围栏与内容门禁；
- 模块维护说明和公开参考不会被解析进最后一题的答案或误区；
- 21 篇小林 Coding 页面全部有唯一 URL、实际题数、质量备注和题目映射；
- 小林新增题不能只有发现页而缺少独立核验来源，专项深化关系必须指向已存在题目；
- 2025 优化修订版 160 题全部有详细答案与独立答题思路，并进入平台和离线版；
- 320 条保留的历史记录全部有审计处置；强匹配与待复核分开统计；
- 旧 HTML 答案不进入公开题库；
...
flowchart LR
    ...
    C["2025 优化修订版大纲<br/>160 道净化补全题"] --> D
    C --> E["legacy-coverage.json"]
    D --> F["questions.json<br/>470 道统一题库"]
    E --> F
    F --> G["FastAPI 查询与组卷"]
...
| `GET` | `/api/v1/legacy-coverage` | 查看 320 条保留历史题的迁移去向 |
