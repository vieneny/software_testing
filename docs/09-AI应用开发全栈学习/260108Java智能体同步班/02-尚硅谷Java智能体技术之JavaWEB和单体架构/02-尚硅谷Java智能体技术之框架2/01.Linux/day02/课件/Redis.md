...
> 课程配图（未随笔记提交）：image-20230627115200495
...
> 课程配图（未随笔记提交）：image-20230627115445004



> 课程配图（未随笔记提交）：image-20230627115514919
...
- 7. 0.10 for Linux（redis-7.0.10.tar.gz）或者安装新版本
  ...
  > 课程配图（未随笔记提交）：image-20230627115822984
...
> 课程配图（未随笔记提交）：image-20230706100628816
...
- 方式1  单key+序列化 .问题:每次修改用户的某个属性需要，先反序列化改好后再序列化回去。开销较大。
  ...
  > 课程配图（未随笔记提交）：image-20230706110321231

- 方式2 多key-value .问题:用户ID数据冗余 &#x20;

  > 课程配图（未随笔记提交）：image-20230706110341117

- 方式3 单key + 多(field+value)

> 课程配图（未随笔记提交）：image-20230706110256007
...
> 课程配图（未随笔记提交）：image-20230627195300172
...
> 课程配图（未随笔记提交）：image-20230706115102403

```纯文本
保存配置，停止服务，重启启动查看进程，不再限制是本机访问了。
```

> 课程配图（未随笔记提交）：image-20230706115212609
...
> 课程配图（未随笔记提交）：image-20230706115349569
...
> 课程配图（未随笔记提交）：image-20230706115554212
...
> 课程配图（未随笔记提交）：image-20230706115650664
...
> 课程配图（未随笔记提交）：image-20230706115754570
...
> 课程配图（未随笔记提交）：image-20230706120047530
...
> 课程配图（未随笔记提交）：image-20230706120131180
...
> 课程配图（未随笔记提交）：image-20230706120319825
...
> 课程配图（未随笔记提交）：image-20230706120451425
...
> 课程配图（未随笔记提交）：image-20230706120627340
...
> 课程配图（未随笔记提交）：image-20230706120811624
...
> 课程配图（未随笔记提交）：image-20230706121200103

**重新连接客户端测试**

> 课程配图（未随笔记提交）：image-20230706121253744
...
> 课程配图（未随笔记提交）：image-20230706121546036
...
> 课程配图（未随笔记提交）：image-20230706121729584
...
> 课程配图（未随笔记提交）：image-20230706121843454
...
> 课程配图（未随笔记提交）：image-20230706122145032
...
> 课程配图（未随笔记提交）：image-20230706135508396
...
> 课程配图（未随笔记提交）：image-20230706135937567
...
> 课程配图（未随笔记提交）：image-20230706140338715
...
> 课程配图（未随笔记提交）：image-20230706141601678
...
> 课程配图（未随笔记提交）：image-20230706154943584
...
> 课程配图（未随笔记提交）：image-20230706155329622
...
> 课程配图（未随笔记提交）：image-20230706160027980

**RedisTemplate.execute需要传入三个值**

> 课程配图（未随笔记提交）：image-20230706160201481
...
> 课程配图（未随笔记提交）：image-20230706162436439
...
> 课程配图（未随笔记提交）：image-20230706162540821



> RDB自动执行快照策略

> 课程配图（未随笔记提交）：image-20230706162725781
...
> 课程配图（未随笔记提交）：image-20230706162910097
...
-   rdbcompression配置
    -   对于存储到磁盘中的快照，可以设置是否进行压缩存储。如果是的话，redis会采用LZF算法进行压缩。如果你不想消耗CPU来进行压缩的话，可以设置为关闭此功能。推荐yes.
        > 课程配图（未随笔记提交）：image-20230706163019916
...
> 课程配图（未随笔记提交）：image-20230706163056568
...
> 课程配图（未随笔记提交）：image-20230706163701368
...
> 课程配图（未随笔记提交）：image-20230706164947931
...
-   正常恢复数据
    - 修改默认的appendonly no，改为yes,开启AOF方式
      ...
      > 课程配图（未随笔记提交）：image-20230706163851658
...
> 课程配图（未随笔记提交）：image-20230706164003278
...
- 新建redis6379.conf
  ...
  > 课程配图（未随笔记提交）：image-20230706184344365
...
> 课程配图（未随笔记提交）：image-20230706185404340



### （5）查看启动服务进程

> 课程配图（未随笔记提交）：image-20230706185335546
...
> 课程配图（未随笔记提交）：image-20230706185249563
...
- 执行完毕再次查看主从配置信息
  ...
  > 课程配图（未随笔记提交）：image-20230706185658801
...
> 课程配图（未随笔记提交）：image-20230706190854461
...
- 创建6379 6380 6381  6389 6390 6391 六个结点的配置文件
  ...
  > 课程配图（未随笔记提交）：image-20230706200218897



### （3）第三步,启动六个服务

> 课程配图（未随笔记提交）：image-20230706200143345

-   组合之前，请确保所有redis实例启动后，nodes-xxxx.conf文件都生成正常。

> 课程配图（未随笔记提交）：image-20230706200315643
...
> 课程配图（未随笔记提交）：image-20230706200816752

输入 yes 继续

> 课程配图（未随笔记提交）：image-20230706200954694
...
> 课程配图（未随笔记提交）：image-20230706201216761



## 9.5 集群的slots

> 课程配图（未随笔记提交）：image-20230706201426691
...
> 课程配图（未随笔记提交）：image-20230706201647379
...
> 课程配图（未随笔记提交）：image-20230706201756159
...
> 课程配图（未随笔记提交）：image-20230706202007674
...
> 课程配图（未随笔记提交）：image-20230706202459470

-   主节点恢复后，主从关系会如何？主节点回来变成从机。

> 课程配图（未随笔记提交）：image-20230706202640635
