...
> 课程配图（未随笔记提交）：image-20230721090940955
...
> 课程配图（未随笔记提交）：image-20230624161556300
...
> 课程配图（未随笔记提交）：image-20230721091801547
...
> 课程配图（未随笔记提交）：image-20230721091835236
...
> 课程配图（未随笔记提交）：image-20230721091858141



## 1.3 工作流程

> 课程配图（未随笔记提交）：image-20230721092104682

> 课程配图（未随笔记提交）：image-20230721092129655
...
> 课程配图（未随笔记提交）：image-20230721101439700
...
> 课程配图（未随笔记提交）：image-20230721092532222
...
> 课程配图（未随笔记提交）：image-20230624163155679
...
> 课程配图（未随笔记提交）：image-20251216093214430
...
> 课程配图（未随笔记提交）：image-20251216093818393

> 课程配图（未随笔记提交）：image-20251216093733330
...
> 课程配图（未随笔记提交）：image-20230721101903404
...
> 课程配图（未随笔记提交）：image-20230624163811030
...
> 课程配图（未随笔记提交）：image-20230624164230054
...
> 课程配图（未随笔记提交）：image-20230624170925571
...
> 课程配图（未随笔记提交）：image-20230624171403235
...
> 课程配图（未随笔记提交）：image-20230624171530387

然后在弹出的表单中，填写配置信息：

> 课程配图（未随笔记提交）：image-20210714164856664
...
> 课程配图（未随笔记提交）：image-20230624200928589
...
> 课程配图（未随笔记提交）：image-20230624203044831
...
> 课程配图（未随笔记提交）：image-20230624203153340
...
> 课程配图（未随笔记提交）：image-20230624203334370

请求了三次，两次出现异常，一次成功。当发现访问服务D的请求异常比例过高时，认为服务D有导致雪崩的风险，会拦截访问服务D的一切请求，形成熔断：

> 课程配图（未随笔记提交）：image-20230624203409785
...
> 课程配图（未随笔记提交）：image-20230624203508014
...
\- 控制台（Dashboard）基于 Spring Boot 开发，打包后可以直接运行，不需要额外的 Tomcat 等应用容器。
 ...
 > 课程配图（未随笔记提交）：image-20230624203655208

具有的特征:

> 课程配图（未随笔记提交）：image-20230624203730680
...
> 课程配图（未随笔记提交）：image-20230624215112184

下载完毕以后就会得到一个jar包

> 课程配图（未随笔记提交）：image-20230624215403344
...
> 课程配图（未随笔记提交）：image-20230624215635555

需要输入账号和密码，默认都是：sentinel

登录后，发现一片空白，什么都没有：因为还没有监控任何服务。另外，sentinel是懒加载的，如果服务没有访问，看不到该服务信息。

> 课程配图（未随笔记提交）：image-20230624215704921
...
> 课程配图（未随笔记提交）：image-20230624220303385
...
> 课程配图（未随笔记提交）：image-20230624220603571
...
> 课程配图（未随笔记提交）：image-20230628090407483
...
> 课程配图（未随笔记提交）：image-20230628115300889
...
> 课程配图（未随笔记提交）：image-20220320111824238

选择流控入门

> 课程配图（未随笔记提交）：image-20220320111955904

10个线程，1秒内运行完，QPS是10，超过了5。

选中**流控入门，QPS<5**右键运行

> 课程配图（未随笔记提交）：image-20220320112040803

注意：不要点击菜单中的执行按钮来运行。

点击查看结果树，理想的请求执行结果应该如下所示：

> 课程配图（未随笔记提交）：image-20220320112142379
...
> 课程配图（未随笔记提交）：image-20230628091856122
...
> 课程配图（未随笔记提交）：image-20230628092034118
...
> 课程配图（未随笔记提交）：image-20230628092515666
...
> 课程配图（未随笔记提交）：image-20230628092751648

4、在Jmeter中进行测试

选择《流控模式-关联》：

> 课程配图（未随笔记提交）：image-20220320114459422

可以看到1000个线程，100秒，因此QPS为10，超过了我们设定的阈值：5

查看http请求：

> 课程配图（未随笔记提交）：image-20230628093126793

请求的目标是/api/user/updateUserById，这样这个端点就会触发阈值。但限流的目标是/api/user/findUserByUserId/1，我们在浏览器访问，可以发现：

> 课程配图（未随笔记提交）：image-20230628093300378
...
> 课程配图（未随笔记提交）：image-20230628095013326
...
> 课程配图（未随笔记提交）：image-20230628094306219

7、添加流控规则

点击users资源后面的流控按钮，在弹出的表单中填写下面信息：

> 课程配图（未随笔记提交）：image-20230628094433574
...
> 课程配图（未随笔记提交）：image-20220320150559229

可以看到这里200个线程，50秒内发完，QPS为4，超过了我们设定的阈值2。

一个http请求是访问/api/user/save

> 课程配图（未随笔记提交）：image-20230628094648097

另一个是访问/api/user/query

> 课程配图（未随笔记提交）：image-20230628094713655

运行测试，察看结果树：

访问/api/user/save,没有进行限流

> 课程配图（未随笔记提交）：image-20230628094814795

访问/api/user/query,进行限流了

> 课程配图（未随笔记提交）：image-20230628094857374
...
> 课程配图（未随笔记提交）：image-20230628095109686
...
> 课程配图（未随笔记提交）：image-20220320152944101
...
> 课程配图（未随笔记提交）：image-20230628095505037

2、jmeter测试

选择《流控效果，warm up》

> 课程配图（未随笔记提交）：image-20220320153409220

QPS为10

刚刚启动时，大部分请求失败，成功的只有3个，说明QPS被限定在3：

> 课程配图（未随笔记提交）：image-20220320153522505

随着时间推移，成功比例越来越高

> 课程配图（未随笔记提交）：image-20220320153646510

到sentinel控制台查看实时监控

> 课程配图（未随笔记提交）：image-20230628095925921
...
> 课程配图（未随笔记提交）：image-20230628100019712

如果使用队列模式做流控，所有进入的请求都要排队，以固定的200ms的间隔执行，QPS会变的很平滑

> 课程配图（未随笔记提交）：image-20230628100049968
...
> 课程配图（未随笔记提交）：image-20230628100313331

2、jmeter测试

> 课程配图（未随笔记提交）：image-20220320154801992

QPS为15，已经超过了我们设定的10。

运行测试用例，察看结果树：

> 课程配图（未随笔记提交）：image-20220320155103019

全部都通过了。

再去sentinel查看实时监控的QPS曲线

> 课程配图（未随笔记提交）：image-20220320155202523
...
> 课程配图（未随笔记提交）：image-20230628100914491

访问/goods/{id}的请求中，id参数值会有变化，热点参数限流会根据参数值分别统计QPS，统计结果：

> 课程配图（未随笔记提交）：image-20230628101012945

当id=1的请求触发阈值被限流时，id值不为1的请求不受影响。

配置方式(点击资源中的热点按钮)：

> 课程配图（未随笔记提交）：image-20230628101216576
...
> 课程配图（未随笔记提交）：image-20230628101331468
...
> 课程配图（未随笔记提交）：image-20230628101715773

这里不要点击hot后面的按钮，页面有BUG

点击左侧菜单中**热点规则**菜单：

> 课程配图（未随笔记提交）：image-20230628102031276

3、jmeter测试

选择《热点参数限流 QPS1》

> 课程配图（未随笔记提交）：image-20220320162420189
...
| 101  | > 课程配图（未随笔记提交）：image-20220320162905731 |
| ---- | ------------------------------------------------------------ |
| 102  | > 课程配图（未随笔记提交）：image-20220320163002195 |
| 103  | > 课程配图（未随笔记提交）：image-20220320163023729 |
...
> 课程配图（未随笔记提交）：image-20230809102346561
...
> 课程配图（未随笔记提交）：image-20230503183655565

> 课程配图（未随笔记提交）：image-20230503183721175
...
> 课程配图（未随笔记提交）：image-20230503185416608
...
> 课程配图（未随笔记提交）：image-20230503191050511
...
> 课程配图（未随笔记提交）：image-20230503191655562
...
> 课程配图（未随笔记提交）：image-20230503213502251
...
> 课程配图（未随笔记提交）：image-20251216100338315
...
> 课程配图（未随笔记提交）：image-20230503222331245
...
> 课程配图（未随笔记提交）：image-20230503223822056
...
> 课程配图（未随笔记提交）：image-20230503224702411
