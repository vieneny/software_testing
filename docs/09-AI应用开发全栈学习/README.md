...
- `260108Java智能体同步班/` 中 77 篇当前 day 的 Markdown 笔记可直接在本地编辑器或 Git 平台阅读，不需要启动服务、数据库或前端项目。
- 前三个模块已包含 1,206 张课程配图，按笔记相对路径存放。重复的“预习资料”目录已移除，下一节内容只在对应的正式 day 中保留。GitHub、Gitee 在线打开 Markdown 即可查看图文；下载完整仓库后，也可在支持 Markdown 预览的本地编辑器中离线阅读。
- HTML 代码块中的示例图片路径保留为教学代码，不作为正文配图。
- 课程原始资料仍保留在本机源目录。仓库维护笔记正文及其配图，不包含视频、压缩包、安装包、IDE 配置或数据库备份。
