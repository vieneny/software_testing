...
- 前三个模块已包含 1,404 张课程配图，按笔记相对路径存放。GitHub、Gitee 在线打开 Markdown 即可查看图文；下载完整仓库后，也可在支持 Markdown 预览的本地编辑器中离线阅读，不需要启动服务。
- 数组预习笔记中有 1 张配图在源资料中缺失，已在原位置说明；HTML 代码块中的示例图片路径保留为教学代码，不作为正文配图。
