# App 自动化项目快速学习：3 天掌握 Appium 项目主线
...
本模块将一套三天 Appium 课程中的通用知识重新组织成可执行学习路线。它不复制课堂工程，而是把环境、定位、业务流、Page Object、pytest、数据驱动、失败证据和 CI 串成一条主线，并直接复用本仓库已经维护的公开/合成练习工程。
...
进入 `labs/automation-practice`，安装基础与开发依赖：
...
使用官方公开练习 App，按 [Android 真机自动化全流程](../03-安卓真机自动化全流程.md)准备环境。最少检查：
...
iOS 自动化需要 macOS、Xcode、XCUITest Driver 和 WDA。先读 [iOS 自动化全流程](../04-苹果移动端自动化全流程.md)，不要在 Windows 上尝试绕过平台限制。
...
本仓库的实现位于 `labs/automation-practice/src/qa_learning/mobile/移动端驱动工厂.py`。它要求绝对 App 路径和明确 UDID，并只接受仓库支持的公开练习 Profile。
...
1. [App 自动化 Mock 框架全流程](../07-移动端自动化模拟框架全流程.md)：没有设备也能运行的框架训练；
2. [Android 真机自动化全流程](../03-安卓真机自动化全流程.md)：ADB、UiAutomator2、公开 APK 与真机证据；
3. [iOS 自动化全流程](../04-苹果移动端自动化全流程.md)：XCUITest、WDA、Simulator 与签名；
4. [框架设计与进阶项目](../05-框架设计与进阶项目.md)：配置、数据、报告、CI 和跨端项目；
5. [故障排查](../06-故障排查.md)：按层定位环境、连接、Session、定位和状态问题。
...
- `labs/automation-practice/src/qa_learning/mobile/mock/`
- `labs/automation-practice/tests/unit/测试_离线移动端*.py`
- `labs/automation-practice/tests/mobile/`
